...
- **UI 扩展点丰富**  
  通过内置的注册中心，你可以动态添加侧边栏按钮、右键菜单项、属性面板扩展槽、浮动搜索过滤器、自定义边类型、内联控件等，无需修改任何核心组件。
...
│   ├── components/       # UI 组件（部分已通过注册中心扩展）
│   ├── hooks/            # 少量自定义 Hook（布局、项目配置）
│   ├── registry/         # 扩展注册中心（侧边栏按钮、右键菜单、属性面板等）
│   └── utils/            # 工具函数、类型定义
