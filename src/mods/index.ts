...
// 新增：防御校验，过滤无效的自定义 Mod 对象（缺少 id 或 init 方法）
//
...
export function initMods(bus: EditorBus, extraMods: EditorMod[] = []): () => void {
    // ==================== 0. 防御校验：确保 extraMods 是数组，并过滤无效 Mod ====================
    const safeExtraMods = Array.isArray(extraMods) ? extraMods : [];
    const validExtraMods = safeExtraMods.filter(mod => {
        if (!mod || typeof mod !== 'object') {
            console.warn('[initMods] 忽略无效的自定义 Mod（非对象）:', mod);
            return false;
        }
        if (typeof mod.id !== 'string' || !mod.id) {
            console.warn('[initMods] 忽略无效的自定义 Mod（缺少 id 或 id 非字符串）:', mod);
            return false;
        }
        if (typeof mod.init !== 'function') {
            console.warn('[initMods] 忽略无效的自定义 Mod（缺少 init 函数）:', mod);
            return false;
        }
        return true;
    });

    if (validExtraMods.length !== safeExtraMods.length) {
        console.warn(`[initMods] 过滤掉 ${safeExtraMods.length - validExtraMods.length} 个无效的自定义 Mod`);
    }
    ...
    for (const mod of validExtraMods) {
